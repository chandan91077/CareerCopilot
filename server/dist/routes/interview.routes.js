...
router.post('/start', auth_middleware_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const firstQuestionText = await openai_service_1.OpenAIService.generateNextQuestion(category, experienceLevel, []);
        const interview = new models_1.Interview({
            user: req.user?.id,
...
router.post('/answer', auth_middleware_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const interview = await models_1.Interview.findOne({ _id: interviewId, user: req.user?.id });
        if (!interview) {
        ...
        const evaluation = await openai_service_1.OpenAIService.evaluateAnswer(activeQuestion.question, activeQuestion.userAnswer || '', interview.category);
        ...
        if (hasNext) {
            ...
            const nextQuestionText = await openai_service_1.OpenAIService.generateNextQuestion(interview.category, interview.experienceLevel, history);
...
router.post('/behavioral', auth_middleware_1.authMiddleware, async (req, res) => {
    const { question, answer } = req.body;
    try {
        if (!question || !answer) {
            return res.status(400).json({ message: 'Question and answer are required' });
        }
        const evalResults = await openai_service_1.OpenAIService.evaluateBehavioralAnswer(question, answer);
...
router.get('/history', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const list = await models_1.Interview.find({ user: req.user?.id }).sort({ createdAt: -1 });
...
router.get('/:id', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const session = await models_1.Interview.findOne({ _id: req.params.id, user: req.user?.id });
        if (!session) {
            return res.status(404).json({ message: 'Interview session not found' });
        }
