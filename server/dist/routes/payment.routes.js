...
router.post('/subscribe', auth_middleware_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        await payment.save();
        // Update User Plan
        await models_1.User.findByIdAndUpdate(req.user?.id, { plan: planType });
        return res.json({
...
router.get('/status', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const sub = await models_1.Subscription.findOne({ user: req.user?.id, status: 'active' }).sort({ createdAt: -1 });
        if (!sub) {
...
router.post('/cancel', auth_middleware_1.authMiddleware, async (req, res) => {
    try {
        const sub = await models_1.Subscription.findOne({ user: req.user?.id, status: 'active' });
        if (!sub) {
            return res.status(404).json({ message: 'No active subscription found' });
        }
        sub.status = 'cancelled';
        await sub.save();
        await models_1.User.findByIdAndUpdate(req.user?.id, { plan: 'free' });
...
router.post('/cashfree/create-order', auth_middleware_1.authMiddleware, async (req, res) => {
...
router.post('/cashfree/verify-order', auth_middleware_1.authMiddleware, async (req, res) => {
    const { orderId } = req.body;
    try {
        if (!orderId) {
            return res.status(400).json({ message: 'Order ID is required' });
        }
        const payment = await models_1.Payment.findOne({ transactionId: orderId, user: req.user?.id }).populate('subscriptionId');
        if (!payment) {
        ...
        if (orderStatus === 'PAID') {
            ...
            if (payment.subscriptionId) {
                const sub = await models_1.Subscription.findById(payment.subscriptionId);
                if (sub) {
                    sub.status = 'active';
                    await sub.save();
                    // Update User plan details
                    await models_1.User.findByIdAndUpdate(req.user?.id, { plan: sub.planType });
                }
            }
