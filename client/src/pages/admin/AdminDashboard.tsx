...
export default function AdminDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [analyticsRes, logsRes] = await Promise.all([
        fetch(getApiUrl('/api/admin/analytics'), { headers }),
        fetch(getApiUrl('/api/admin/logs'), { headers })
      ]);
  ...
  const statCards = [
    { title: 'Total Registered Users', value: data?.totalUsers || 0, icon: Users, color: 'text-indigo-500 bg-indigo-500/10' },
    { title: 'Premium Subscribers', value: data?.premiumUsers || 0, icon: CreditCard, color: 'text-purple-500 bg-purple-500/10' },
    { title: "Today's Interview practices", value: data?.todayInterviewsCount || 0, icon: Play, color: 'text-emerald-500 bg-emerald-500/10' },
    { title: 'Total Revenue ($)', value: `$${data?.totalRevenue || 0}`, icon: ShieldCheck, color: 'text-amber-500 bg-amber-500/10' }
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">Review active user metrics, monthly revenue trends, OpenAI API costs, and administrator audit logs.</p>
        </div>
        <button 
          onClick={loadData}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((card, idx) => {
          ...
          return (
            <div key={idx} className="bg-white dark:bg-dark-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Revenue Area Chart */}
        <div className="bg-white dark:bg-dark-900 border border-slate-200 dark:border-slate-800/80 rounded-3xl p-6 shadow-sm">
          <div className="mb-6">
            <h3 className="text-base font-bold text-slate-900 dark:text-white">Monthly Revenue Stream</h3>
            <p className="text-xs text-slate-400 mt-0.5">Historical monthly billing summation</p>
          </div>
          <div className="h-64">
        ...
        <div className="bg-white dark:bg-dark-900 border border-slate-200 dark:border-slate-800/80 rounded-3xl p-6 shadow-sm">
          <div className="mb-6">
            <h3 className="text-base font-bold text-slate-900 dark:text-white">OpenAI Usage Cost Breakdown</h3>
            <p className="text-xs text-slate-400 mt-0.5">Cost details split by AI parsers and graders</p>
          </div>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="bg-white dark:bg-dark-900 border border-slate-200 dark:border-slate-800/80 rounded-3xl p-6 shadow-sm">
        <div className="flex items-center gap-2 mb-6 border-b border-slate-200 dark:border-slate-800 pb-4">
          ...
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">Administrative Logs (Audit Trail)</h3>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/40 text-slate-700 dark:text-slate-300">
                {logs.map((log) => (
                  <tr key={log._id} className="hover:bg-slate-50 dark:hover:bg-dark-800/50">
                    <td className="py-3 font-semibold">{log.adminEmail}</td>
                    <td className="py-3 text-indigo-600 dark:text-indigo-400 font-semibold">{log.action}</td>
